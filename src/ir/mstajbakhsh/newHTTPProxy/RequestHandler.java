package ir.mstajbakhsh.newHTTPProxy;

import ir.mstajbakhsh.Variable;
...
public class RequestHandler implements Runnable {
    ...
    public void run() {
        ...
        if (request.equals("CONNECT")) {
            ...
            if ((file = Proxy.getCachedPage(urlString)) != null) {
                ...
                sendNonCachedToClient(urlString, header);
            }
        }
    }
    ...
    private void sendNonCachedToClient(String urlString, String header) {
        boolean isOnion = false;
        java.net.Proxy torProxy = null;
        if (urlString.contains(".onion")) {
            isOnion = true;
            torProxy = new java.net.Proxy(java.net.Proxy.Type.SOCKS, new InetSocketAddress(Variable.localTorIP, Variable.localTorPort));
        }
        ...
        try {
            ...
            if ((fileExtension.contains(".png")) || fileExtension.contains(".jpg") ||
                ...
                if (isOnion) {
                    connection = remoteURL.openConnection(torProxy);
                } else {
                    connection = remoteURL.openConnection();
                }

                //Send previous headers
            ...
            // File is a text file
                ...
                if (isOnion) {
                    proxyToServerCon = (HttpURLConnection) remoteURL.openConnection(torProxy);
                    proxyToServerCon.setConnectTimeout(0);
                } else {
                ...
                for (String h : header.split("\r\n")) {
                    if (h != "") {
                        proxyToServerCon.setRequestProperty(h.split(":")[0].trim(), h.split(":")[1].trim());
                    }
                }

                //Send previous headers
                proxyToServerCon.getOutputStream().write(header.getBytes());
                proxyToServerCon.getOutputStream().flush();

                // Create Buffered Reader from remote Server
                BufferedReader proxyToServerBR;
                if (proxyToServerCon.getResponseCode() < 200 || proxyToServerCon.getResponseCode() >= 300) {
                    //for other codes including 401, ...
                    proxyToServerBR = new BufferedReader(new InputStreamReader(proxyToServerCon.getErrorStream()));
                } else {
                ...
                //String line = "HTTP/1.0 200 OK\n" +
                //String line = "HTTP/1.0 " + proxyToServerCon.getResponseCode() + " " + proxyToServerCon.getResponseMessage() + "\r\n" +
                //        "Proxy-agent: ProxyServer/1.0\r\n" +
                //        "\r\n";
                //proxyToClientBw.write(line);
                ...
                proxyToClientBw.write(line);
                //Send headers
                proxyToServerCon.getHeaderFields().forEach((h,v) -> {
                    try {
                        if (h != null) {
                            String value = v.toString();
                            value = value.replaceAll("\\[", "").replaceAll("]", "");
                            String line2 = h + ": " + value + "\r\n";
    ...
    private void handleHTTPSRequest(String urlString) {
        ...
        try {
            ...
            java.net.Proxy torProxy = null;
            if (isOnion) {
                InetSocketAddress hiddenSA;
                hiddenSA = InetSocketAddress.createUnresolved(url, port);

                address = hiddenSA.getAddress();
                torProxy = new java.net.Proxy(java.net.Proxy.Type.SOCKS, new InetSocketAddress(Variable.localTorIP, Variable.localTorPort));
            ...
            if (isOnion) {
                proxyToServerSocket = new Socket(torProxy);
                proxyToServerSocket.connect(new InetSocketAddress(url, port), 0);
            } else {
